...
def verify_student(intent_request):
    ...
    str=check_student(email,order,rating)
    return close(
        output_session_attributes,
        'Fulfilled',
        {
...
def check_student(email,restaurant,rating):
    try:
        request_data={'email': email,'restaurant':restaurant,rating:'rating'}
        result=requests.post('https://gbevkqen2jubinclkwjepekqv40hbtay.lambda-url.us-east-1.on.aws/',json=request_data)
        return result.text
